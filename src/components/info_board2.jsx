...
export function InfoBoard({ faction, type = "lord" }) {
  ...
  useEffect(() => {
    ...
    gsap.set(imgRef.current, { x: -200, opacity: 0 });
              ...
              <div className="grid grid-rows-2 md:grid-cols-2 w-full h-fit">
                <div className="grid grid-cols-6 w-full h-fit">
                  <div className="col-span-2 text-right text-[#C4C4C4]">
                    ...
                    <p>{lang("fi.size")}</p>
                    <p>{lang("fi.type")}</p>
                    {type === "soldier" ? <p>{lang("fi.cost")}</p> : null}
                  </div>
                  <div className="col-span-4 ml-2">
                    ...
                    {type === "soldier" ? <p>{lang(obj[selected].cost)}</p> : null}
                  </div>
                </div>

                {/* --------------- 技能列表开始 --------------- */}
                <div key={selected} className="flex w-full h-fit" ref={skillsContainerRef}>
                  <div className="flex flex-wrap gap-4 ml-[10vw] md:ml-0">
                    {(obj[selected]?.skills ?? []).map((skill, skillIndex) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenSkillKey((prev) => (prev === uniqueKey ? null : uniqueKey));
                          }}
                        >
                          {/* 技能图标 */}
                          {iconSrc && (
                          ...
                          >
                            <p className="font-bold leading-tight text-[20px]">
                              {lang(skill.name, { defaultValue: `${lang("nullSkill")} ${skillIndex + 1}` })}
                            </p>
